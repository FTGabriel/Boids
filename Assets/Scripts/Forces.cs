...
public class Forces: MonoBehaviour
{
    ...
    // Determine the barycentre based on the number of boids and their transform
    // Then calculate the distance between them, making the flock looks differently with cohesion, alignement and separation
    // Boids are pulled to the center, updating their speed and position
    ...
    private Vector3 ApplyProfile(Vector3 cohesion, Vector3 alignment, Vector3 separation, GameObject boidObj)
    {
        ...
        switch (boid.profile)
        {
            ...
            case BoidProfile.Happy:
                cohesion *= Random.Range(-3f, 1.5f);
                alignment *= Random.Range(-3f, 1.5f);
                separation *= Random.Range(-3f, 1.5f);
                SetBoidColor(boidObj, Color.yellow);
                break;

            case BoidProfile.Annoying:
                separation *= Random.Range(0.2f,1f); // Don't avoid a lot collisions
                SetBoidColor(boidObj, Color.green);
                break;

            default: // Normal
                SetBoidColor(boidObj, Color.red);
                break;
        }
    ...
    // Allows the boids to get a color
